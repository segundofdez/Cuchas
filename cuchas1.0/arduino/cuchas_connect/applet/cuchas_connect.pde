...
// VALORES DE POTENCIOMETROS: almacena los valores de entradas analogicas
int valPotX=0;
int valPotY=0;
int valPotR=0;
int valPotG=0;
...
int potX=0; //las variabes que dividiran y seran mandadas por serial
int potY=0;
int potR=0;
int potG=0;
int potB=0;
...
void loop(){
  ...
  if (Serial.available() > 0) {  //si llega algun dato de processing
    ...
    if (valBoton1)
      serialWrite('H'); //hight si esta pulsado 1
    else 
      serialWrite('L'); //sino Low
    delay(50);

    if (valBoton2)
      serialWrite('H'); //hight si esta pulsado 2 
    else 
      serialWrite('L'); //sino Low
    delay(50);

    if (valBoton3)
      serialWrite('H'); //hight si esta pulsado 3 
    else 
      serialWrite('L'); //sino Low
    delay(50);

    if (valBoton4)
      serialWrite('H'); //hight si esta pulsado 4
    else 
      serialWrite('L'); //sino Low
    delay(50);
  }
